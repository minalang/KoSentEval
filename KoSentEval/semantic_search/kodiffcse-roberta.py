...
def get_embeddings(text):
    # 모델 설정
    model = AutoModel.from_pretrained('BM-K/KoDiffCSE-RoBERTa').to(device)  # or 'BM-K/KoSimCSE-roberta-multitask'
    tokenizer = AutoTokenizer.from_pretrained('BM-K/KoDiffCSE-RoBERTa')  # or 'BM-K/KoSimCSE-roberta-multitask'
    # 텍스트 처리
...
def main():
    ...
    f = open('./result/kodiffcse-roberta.txt', 'w')
    ...
    print(result_csv)
    f.close()
    result_csv.to_csv('./result/kodiffcse-roberta.csv', encoding = 'utf-8-sig')
