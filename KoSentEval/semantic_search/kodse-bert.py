...
def get_embeddings(text):
    # 모델 설정
    model = AutoModel.from_pretrained('minalang/KoDSE-bert').to(device)  # or 'BM-K/KoSimCSE-roberta-multitask'
    tokenizer = AutoTokenizer.from_pretrained('minalang/KoDSE-bert')  # or 'BM-K/KoSimCSE-roberta-multitask'
    # 텍스트 처리
...
def main():
    ...
    f = open('./result/kodse-bert.txt', 'w')
    ...
    print(result_csv)
    f.close()
    result_csv.to_csv('./result/kodse-bert.csv', encoding = 'utf-8-sig')
