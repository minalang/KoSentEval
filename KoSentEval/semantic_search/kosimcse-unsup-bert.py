...
def get_embeddings(text):
    # 모델 설정
    model = AutoModel.from_pretrained('BM-K/KoSimCSE-Unsup-BERT').to(device)  # or 'BM-K/KoSimCSE-roberta-multitask'
    tokenizer = AutoTokenizer.from_pretrained('BM-K/KoSimCSE-Unsup-BERT')  # or 'BM-K/KoSimCSE-roberta-multitask'
    # 텍스트 처리
...
def main():
    ...
    f = open('./result/kosimcse-unsup-bert.txt', 'w')
    ...
    print(result_csv)
    f.close()
    result_csv.to_csv('./result/kosimcse-unsup-bert.csv', encoding = 'utf-8-sig')
