...
def get_embeddings(text):
    # 모델 설정
    model = AutoModel.from_pretrained('sentence-transformers/LaBSE').to(device)  # or 'BM-K/KoSimCSE-roberta-multitask'
    tokenizer = AutoTokenizer.from_pretrained('sentence-transformers/LaBSE')  # or 'BM-K/KoSimCSE-roberta-multitask'
    # 텍스트 처리
...
def main():
    ...
    print(result_csv)
    f.close()
    result_csv.to_csv('./result/labse.csv', encoding = 'utf-8-sig')
