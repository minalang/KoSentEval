...
def get_embeddings(text):
    # 모델 설정
    model = AutoModel.from_pretrained('BM-K/KoSimCSE-bert').to(device)  # or 'BM-K/KoSimCSE-roberta-multitask'
    tokenizer = AutoTokenizer.from_pretrained('BM-K/KoSimCSE-bert')  # or 'BM-K/KoSimCSE-roberta-multitask'
    # 텍스트 처리
...
def main():
    ...
    f = open('./result/kosimcse-bert.txt', 'w')
    ...
    print(result_csv)
    f.close()
    result_csv.to_csv('./result/kosimcse-bert.csv', encoding = 'utf-8-sig')
